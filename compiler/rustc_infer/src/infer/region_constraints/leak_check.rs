    ...
    fn iterate_region_constraints(
        ...
        if let Some(snapshot) = only_consider_snapshot {
                ...
                region_constraints.undo_log.region_constraints_in_snapshot(&snapshot.undo_snapshot)
            {
                match undo_entry {
                    &AddConstraint(i) => {
                        let constraint = region_constraints.data().constraints[i].0;
                        each_edge(constraint.sub, constraint.sup);
                    }
            ...
            region_constraints
                ...
                .constraints
                .iter()
                .for_each(|(constraint, _)| each_edge(constraint.sub, constraint.sup))
        }
    }
