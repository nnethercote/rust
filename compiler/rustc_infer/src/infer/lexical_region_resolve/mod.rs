        ...
        for (constraint, _) in &self.data.constraints {
            match constraint.kind {
                ConstraintKind::RegSubVar => {
                    ...
                    if self.expand_node(constraint.sub, sup_vid, sup_data) {
                ...
                ConstraintKind::VarSubVar => {
                    ...
                    match *var_values.value(sub_vid) {
                        ...
                        VarValue::Empty(a_universe) => {
                            ...
                            let changed = match *sup_data {
                                VarValue::Empty(sup_universe) => {
                                    ...
                                    let ui = a_universe.min(sup_universe);
                        ...
                        VarValue::Value(a_region) => {
                            ...
                            if self.expand_node(a_region, sup_vid, sup_data) {
                                changes.push(sup_vid);
                            }
                            match sup_data {
                                VarValue::Value(Region(Interned(ReStatic, _)))
        ...
        for (constraint, origin) in &self.data.constraints {
            ...
            match constraint.kind {
                ...
                ConstraintKind::RegSubReg => {
                    if self.sub_concrete_regions(constraint.sub, constraint.sup) {
                        continue;
                    }
                    ...
                    debug!(
                        "region error at {:?}: \
                         cannot verify that {:?} <= {:?}",
                        origin, constraint.sub, constraint.sup
                    );
                ...
                ConstraintKind::VarSubReg => {
                    ...
                    debug!("contraction: {:?} == {:?}, {:?}", sub_vid, sub_data, constraint.sup);
                    ...
                    // collect them later.
                    if !self.sub_concrete_regions(sub_region, constraint.sup) {
                        debug!(
                            "region error at {:?}: \
                            cannot verify that {:?}={:?} <= {:?}",
                            origin, sub_vid, sub_region, constraint.sup
                        );
    ...
    fn construct_graph(&self) -> RegionGraph<'tcx> {
        ...
        for (constraint, _) in &self.data.constraints {
            match constraint.kind {
                ConstraintKind::VarSubVar => {
                    graph.add_edge(
                        NodeIndex(constraint.sub.as_var().index()),
                        NodeIndex(constraint.sup.as_var().index()),
                        *constraint,
                    );
                }
                ConstraintKind::RegSubVar => {
