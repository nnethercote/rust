    ...
    for (constraint, _) in &regions.constraints {
        match constraint.kind {
            ...
            ConstraintKind::RegSubVar => {
                ...
                deps.smaller.insert(RegionTarget::Region(constraint.sub));
            }
            ConstraintKind::VarSubReg => {
                let sub_vid = constraint.sub.as_var();
                let deps = map.entry(RegionTarget::RegionVid(sub_vid)).or_default();
                deps.larger.insert(RegionTarget::Region(constraint.sup));
            }
            ConstraintKind::RegSubReg => {
                // The constraint is already in the form that we want, so we're done with it
                // The desired order is [larger, smaller], so flip them.
                if early_bound_region_name(constraint.sub)
                    != early_bound_region_name(constraint.sup)
                {
                    outlives_predicates
                        .entry(
                            early_bound_region_name(constraint.sup).expect("no region_name found"),
                        )
                        .or_default()
                        .push(constraint.sub);
                }
